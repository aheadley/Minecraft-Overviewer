...
import json
import util
from render.c_overviewer import get_render_mode_inheritance, get_render_mode_info
import overviewer_version
from minecraft import RegionSet
...
def mirror_dir(src, dest, entities=[]):
    """Copy all entities from src to dst
    """
    ...
    for entry in os.listdir(src):
        if  entry in MIRROR_SKIP_FILES or \
            entry not in entities:
            continue

        if os.path.isdir(os.path.join(src,entry)):
            mirror_dir(os.path.join(src, entry), os.path.join(dest, entry))
...
class MapGenerator(object):
    ...
    def __init__(self, dest_path, **kwargs):
        ...
        DEFAULT_WEB_ASSETS_PATH     = os.path.join(util.get_program_path(),
            'overviewer_core', 'data', 'web_assets')
        #TODO this is probably not the right color
        DEFAULT_BG_COLOR            = '#000000'

        self.dest_path = dest_path
        self.web_assets_hook = kwargs.get('web_assets_hook')
        self.web_assets_path = kwargs.get('web_assets_path',
            self.DEFAULT_WEB_ASSETS_PATH)
        self.bg_color = self._parse_color_string(kwargs.get('bg_color',
            self.DEFAULT_BG_COLOR))
        self.north_direction = kwargs.get('north_direction',
            RegionSet.DEFAULT_DATA.get('north_direction'))

    def initialize_interface(self, quadtrees):
        """Writes out config.js, marker.js, and region.js
        Copies web assets into the destdir
        """
        #TODO this whole class needs a lot of work
        for qt in quadtrees:
            blank_img = Image.new('RGBA', (1,1), qt.get_background_color())
    ...
    def _parse_color_string(self, color, alpha=0):
        """
        """
        try:
            parsed_alpha = int(color[7:9], 16) & 0xFF
        except IndexError:
            parsed_alpha = alpha
        return (
